...
### Will it run on Vercel or Netlify?

**No.** Those are serverless: the filesystem is read-only and wiped between
requests, and there is no long-lived process. This app keeps a SQLite file and
holds server-side MFA state, so it needs an ordinary always-on Node server.

Use **Render**, **Railway**, or **Fly.io**. The steps below are for Render.

### Deploy to Render

1. **Push the code to GitHub.** `.env` and `data/` are git-ignored, so no secret
   and no database file is uploaded.

2. **New → Blueprint**, point Render at the repository. It reads
   [`render.yaml`](render.yaml) and creates the service, the persistent disk, and
   the environment variables.

3. **Fill in the three secrets it asks for** (they are marked `sync: false` so
   they never live in the repo):

   | Variable | Value |
   |---|---|
   | `EMAIL_USER` | `guptavipul2726@gmail.com` |
   | `EMAIL_PASSWORD` | your 16-character Gmail App Password |
   | `EMAIL_FROM` | `Aman <guptavipul2726@gmail.com>` |

   `SESSION_SECRET` and `HASH_PEPPER` are generated by Render automatically.

4. **Deploy.** Render sets `APP_BASE_URL` to the real public address, which is
   what the "YES, IT'S ME" link is built from.

### What the deployment needs, and why

| Setting | Value | Why it matters |
|---|---|---|
| `NODE_ENV` | `production` | Forces `DEMO_MODE` off and marks cookies `secure` |
| `APP_BASE_URL` | the public **https** URL | The email link is built from it; a wrong value produces links nobody can open |
| `DATA_DIR` | the mounted disk, e.g. `/var/data` | Without it the database sits on the container's temporary filesystem and all accounts vanish on restart |
| `NODE_VERSION` | `22.16.0` or newer | `node:sqlite` needs Node 22.5+ |
| Health check | `/healthz` | Returns `{"status":"ok"}` and confirms the database is reachable |

The server **refuses to start** in production if `APP_BASE_URL` is still
localhost, is not https, or if `MAIL_TRANSPORT` is not `smtp` — these are exactly
the mistakes that only reveal themselves later, inside somebody's inbox.

### Free plan warning

Render's free plan has **no persistent disk**, so registered accounts are lost
whenever the service restarts (including after 15 minutes of inactivity). That is
usually fine for a demo — you register live in front of the evaluator anyway — but
delete the `disk:` block from `render.yaml` if you use it, and leave `DATA_DIR`
unset.

### Sending mail from a server

Gmail allows roughly 500 messages a day and works from most hosts. If your
provider blocks outbound SMTP, switch to **Brevo** — change only the `EMAIL_*`
variables in the dashboard, no code change.
