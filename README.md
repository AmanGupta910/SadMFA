...
```
SAD/
├── server.js                    local entry point (opens a port)
├── api/index.js                 Vercel entry point (exports the app)
├── vercel.json                  routes every request to the function
├── render.yaml                  optional always-on alternative
├── package.json
├── .env                         real settings (git-ignored, never commit)
├── .env.example                 safe template with dummy values
│
├── src/
│   ├── app.js                   builds the Express app (shared by both entries)
│   ├── config/env.js            loads + validates every setting
│   ├── db/
│   │   ├── index.js             one async API over SQLite *or* Neon Postgres
│   │   ├── schema.sql           table definitions (SQLite)
│   │   ├── schema.postgres.sql  the same tables in Postgres dialect
│   │   ├── sessionStore.js      database-backed express-session store
...
│   ├── check-email.js           npm run check-email  (login only, sends nothing)
│   ├── test-email.js            npm run test-email
│   └── start-offline.js         npm run start:offline (no-internet fallback)
├── tests/
│   ├── mfa-flow.test.js         59 end-to-end checks
│   └── postgres-dialect.test.js 20 checks that the Postgres SQL is valid
└── data/                        SQLite database (local only, git-ignored)
```
...
### PostgreSQL check (no server, no account needed)

```bash
npm run test:postgres
```

**20 checks** that run the real `schema.postgres.sql` against a genuine
PostgreSQL engine compiled to WebAssembly. This proves the deployed database
dialect works — identity columns, `RETURNING`, `ON CONFLICT`, foreign keys,
`LIMIT ?`, and text timestamp comparison — before anything reaches Vercel.
...
The app runs on Vercel as a serverless function. Two things differ from local:

* **`api/index.js`** exports the Express app instead of opening a port
  (`vercel.json` routes every request to it).
* **The database moves to Neon Postgres.** Serverless has no persistent disk and
  each request can hit a different instance, so a SQLite *file* cannot be used.
  The code supports both: SQLite locally, Postgres whenever `DATABASE_URL` is
  set. Nothing else changes.

### Step 1 - push to GitHub

`.env` and `data/` are git-ignored, so no secret and no database file is uploaded.

### Step 2 - import into Vercel

**Add New → Project**, pick the repository, **Deploy**. The first build will
succeed but the site will error until the database exists - that is next.

### Step 3 - attach the Neon database

In the project: **Storage → Create Database → Neon (Postgres) → Create**.

Vercel injects `DATABASE_URL` automatically. Nothing to copy or paste, and the
tables are created by the app on its first request.

### Step 4 - add the environment variables

**Settings → Environment Variables**. Generate the two secrets with:
...
|---|---|
| `NODE_ENV` | `production` |
| `APP_BASE_URL` | your real URL, e.g. `https://gocart-mfa.vercel.app` |
| `SESSION_SECRET` | a 64-character random string |
| `HASH_PEPPER` | a different 64-character random string |
| `MAIL_TRANSPORT` | `smtp` |
...
| `EMAIL_FROM` | `GoCart Security <your-address@gmail.com>` |

Then **Redeploy** so the variables take effect.

> `APP_BASE_URL` must be the real **https** address: the "YES, IT'S ME" link is
> built from it. The app refuses to boot in production if it is still localhost
> or is not https, because that mistake only shows up inside someone's inbox.
...
`"driver": "postgres"` confirms it is using Neon. If it says `sqlite`, then
`DATABASE_URL` is missing and data would vanish between requests.

### Things worth knowing

| Topic | Detail |
|---|---|
| Free tier | Neon's free plan sleeps when idle; the first request after that takes a second or two to wake it |
| Function timeout | Set to 30s in `vercel.json` - plenty, since sending an email takes 1-3s |
| SMTP | Vercel's Node runtime allows outbound SMTP, so nodemailer works. If your provider ever blocks it, switch to Brevo by changing only the `EMAIL_*` variables |
| Local development | Unchanged. Without `DATABASE_URL` it still uses the local SQLite file, so `npm start` and `npm test` work exactly as before |
| Render | [`render.yaml`](render.yaml) is still included if you ever want a always-on server with SQLite instead |
...
| `npm test` | Run the 59 end-to-end checks (needs `start:offline` running) |
| `npm run test:postgres` | Verify the Postgres schema works (20 checks, standalone) |
